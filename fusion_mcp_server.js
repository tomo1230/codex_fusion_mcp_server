    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(ListToolsRequestSchema, async () => {
            ...
            const tools = [
                ...
                {
                    name: 'export_sat',
                    description: '指定したボディを SAT フォーマットファイルとしてエクスポートします。',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            body_name: {
                                type: 'string',
                                description: 'エクスポート対象のボディ名。'
                            },
                            file_path: {
                                type: 'string',
                                description: '出力先の SAT ファイルパス。'
                            },
                            overwrite: {
                                type: 'boolean',
                                default: false,
                        ...
                        required: ['body_name', 'file_path']
                    }
                },
                {
                    name: 'import_sat',
                    description: 'SAT フォーマットファイルを読み込んで Fusion 360 にインポートします。',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            file_path: {
                                type: 'string',
                                description: '読み込む SAT ファイルパス。'
                            },
                            body_name: {
                                type: 'string',
                                description: 'インポート後に付与するボディ名。'
                            },
                            component_name: {
                                type: 'string',
                                description: 'インポート先コンポーネント名。省略時は現在のアクティブコンポーネント。'
                            }
                        },
                        required: ['file_path']
                    }
